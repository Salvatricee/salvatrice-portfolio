        ...
        <div className="grid md:grid-cols-3 gap-6 mb-20">
          {coreSkills.map((skill, index) => (
            <AnimatedSection key={skill.id} delay={0.15 + index * 0.05}>
              <div className="h-full flex flex-col p-8 rounded-2xl bg-white border-2 border-primary-blue/20 shadow-sm hover:shadow-md hover:border-primary-blue/40 transition-all duration-300">
            ...
            <AnimatedSection key={skill.id} delay={0.25 + index * 0.03}>
              <div className="h-full flex flex-col p-5 rounded-xl bg-surface border border-line hover:border-primary-blue/20 transition-colors duration-300">
                ...
                <div className="flex flex-wrap gap-1">
                  {skill.tags.map((tag) => (
                    <span key={tag} className="px-2 py-0.5 text-[11px] font-medium rounded-full bg-white text-muted border border-line">
                      {tag}
                    </span>
                  ))}
                </div>
